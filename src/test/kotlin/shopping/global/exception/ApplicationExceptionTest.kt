    ...
    Given("예외 인스턴스 변수가 생성될 때") {
        When("전달 받은 message 가 있다면") {
            Then("전달 받은 message 를 프로퍼티로 갖는다") {
                val applicationException = ApplicationException(ErrorCode.DUPLICATED_REGISTER_EMAIL, message = "이미 존재 합니다.")
        ...
        Then("에러 로그를 생성한다") {
            ...
            outputStream.toString().trimIndent() shouldContain """
            cause: null
            message: 이미 존재 하는 이메일 입니다.
            errorCode: DUPLICATED_REGISTER_EMAIL""".trimIndent()
        }
    }
})
