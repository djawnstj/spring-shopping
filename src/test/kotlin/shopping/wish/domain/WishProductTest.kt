...
class WishProductTest : BehaviorSpec({
    Given("다른 인스턴스 변수의 위시 리스트와 동등한 객체인지 확인할 때") {
        ...
        val wishProduct = WishProductFixture.`고객 1의 위시 리스트에 저장 된 상품 1`.`엔티티 생성`(product)

        When("해당 객체의 상품, 위시 리스트 와 동일하다면") {
            val sameWishProduct = WishProductFixture.`고객 1의 위시 리스트에 저장 된 상품 1`.`엔티티 생성`(product)
