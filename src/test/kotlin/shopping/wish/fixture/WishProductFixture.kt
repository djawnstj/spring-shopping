...
enum class WishProductFixture(
    ...
    // 정상 상품
    `고객 1의 위시 리스트에 저장 된 상품 1`(ProductFixture.`상품 1`),

    // 비정상 상품
    `상품 정보 null`(null),
    ;

    fun `엔티티 생성`(): WishProduct = WishProduct(product!!.`상품 엔티티 생성`())
    fun `엔티티 생성`(product: Product): WishProduct = WishProduct(product)
